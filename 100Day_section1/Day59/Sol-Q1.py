...
Algorithm
    1.  Do the pre-processing on the given wordList and find all the possible generic/intermediate
        states. Save these intermediate states in a dictionary with key as the intermediate word and 
        value as the list of words which have the same intermediate word.
    2.  Push a tuple containing the beginWord and 1 in a queue. The 1 represents the level number 
        of a node. We have to return the level number of a node. We have to return the level of the endNode
    ...
    5.  Find all the generic transformations of the current_word and find out if any of these transformations
        of other words in the word list. This is achieved by checking the all_combo_dict.
    ...
    ternimation condition for standard BFS is finding the end word
